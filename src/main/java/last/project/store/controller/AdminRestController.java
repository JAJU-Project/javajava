...
public class AdminRestController {
    ...
    @ResponseBody
    public List<SalesVo> salesByMenu(HttpSession session) { // 상품별 매출 실적(10/24)
    ...
    @ResponseBody
    public int dayCustomer(HttpSession session) { // 금일 고객수(10/24)
        String scode = (String) session.getAttribute("scode");
    ...
    @RequestMapping("age")
    @ResponseBody
    public JSONObject age() { // 나이별 고객수(10/24)
        List<Integer> alist = kakaoService.selectAge();
        log.info("#age alist: " + alist);
    ...
    @RequestMapping("comment") // 댓글 seq값 저장(10/24)
    @ResponseBody
    ...
    @ResponseBody
    public String selectCname(long catgo) {
        log.info("selectCname :catgo: " + catgo);
        long caseq = catgo;
        String cname = categoryService.selectByCaseq(caseq);

        log.info("selectCname cname: " + cname);
        return cname;
    }

    @RequestMapping("categoClick")
    @ResponseBody
    public HashMap<String, Object> categoClick(long caseq) {
